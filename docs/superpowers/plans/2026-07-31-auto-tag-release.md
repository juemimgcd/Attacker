...
**Goal:** Create one incrementing semantic-version tag and one GitHub Release whenever a pull request is merged into `master`.

**Architecture:** A single GitHub Actions workflow listens for merged pull requests and optional manual dispatches. It tags the exact merge commit, retries tag allocation if concurrent merges race, reuses an existing tag on reruns, and creates idempotent GitHub Releases with generated notes.
...
- starts at `v0.1.0` when the repository has no semantic-version tags;
...
Expected: no whitespace errors; the diff contains only the requested workflow behavior.
