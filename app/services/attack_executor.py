...
class AttackExecutor:
    # 执行单条攻击样本并返回完整运行结果。
    async def run_once(self, target: TargetConfig, sample: AttackSample) -> AttackRunResult:
        request_body, target_response = await http_target_connector.call(
            target=target,
            prompt=sample.prompt,
        )
        judge_result = judge_engine.judge(sample=sample, target_response=target_response)
